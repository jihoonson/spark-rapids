...
class AsyncWriter2Suite extends AnyFunSuite with BeforeAndAfterAll {

  // TODO: close after all tests
  var asyncWriter: AsyncWriter2 = _

  override def beforeAll(): Unit = {
    asyncWriter = new AsyncWriter2(2)
  }
  ...
  test("test schedule multiple tasks for the same stream and wait for them to flush") {
    ...
    try {
      // Schedule 10 tasks
      for (_ <- 0 until 10) {
        f = scheduleTask(new SleepTask(id, 100))
      }
      Await.ready(f, Duration(2000, MILLISECONDS))
    } finally {
      asyncWriter.deregister(id)
      Thread.sleep(1000)
    }
  ...
  test("test schedule multiple tasks for various streams and wait for one stream to flush") {
    ...
    try {
      ...
      for (_ <- 0 until 5) {
        f = scheduleTask(new SleepTask(id1, 100))
      }
      ...
      Await.ready(f, Duration(600, MILLISECONDS))
    } finally {
...
//  }
  ...
  test("test processing tasks in order") {
    ...
    var f1: Future[Unit] = null
    var f2: Future[Unit] = null
    try {
      for (i <- 0 until 10) {
        f1 = scheduleTask(new OrderedTask(id1, 100, i, q1))
        f2 = scheduleTask(new OrderedTask(id2, 100, i, q2))
      }
      Await.ready(f1, Duration(2000, MILLISECONDS))
      assert(q1.size() == 10)
      for (i <- 0 until 10) {
        assertResult(i)(q1.poll())
      }
      Await.ready(f2, Duration(200, MILLISECONDS))
      assert(q2.size() == 10)
      for (i <- 0 until 10) {
