...
# Unity Catalog 0.5.0 replaced the single unitycatalog-spark_<scala> connector with one artifact
# per Spark line, so earlier versions cannot be resolved by the coordinates below.
[[ "$(printf '%s\n0.5.0\n' "$UC_VERSION" | sort -V | head -n 1)" == "0.5.0" ]] ||
  die "Unity Catalog $UC_VERSION is not supported, 0.5.0 or later is required"
...
resolve_classpath() {
  ...
  "${MVN:-mvn}" "${mvn_args[@]}" >&2 || die "failed to resolve $coordinates"
  rm -rf "$work_dir"
