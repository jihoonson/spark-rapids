...
The suite only applies to a narrow combination: Scala 2.13, Delta Lake 4.2.0, and Spark 4.0.1 or
4.1.1. Unity Catalog 0.5.0 or later is required, because 0.5.0 replaced the single
`unitycatalog-spark_<scala>` connector with one artifact per Spark line.
...
`jenkins/spark-tests.sh` runs the same suite under `TEST_MODE=DELTA_LAKE_UC_ONLY`, with its own
copy of the server launch.
