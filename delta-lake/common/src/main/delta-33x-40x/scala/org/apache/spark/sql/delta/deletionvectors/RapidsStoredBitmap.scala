/*
 * Copyright (c) 2026, NVIDIA CORPORATION.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.apache.spark.sql.delta.deletionvectors

import ai.rapids.cudf.HostMemoryBuffer
import com.nvidia.spark.rapids.Arm.withResource
import com.nvidia.spark.rapids.delta.common.RapidsDeletionVectorStore
import java.io.{DataInputStream, InputStream, IOException}

import org.apache.spark.internal.Logging
import org.apache.spark.sql.delta.actions.DeletionVectorDescriptor
import org.apache.spark.sql.delta.storage.dv.DeletionVectorStore
import org.apache.hadoop.fs.Path

case class RapidsDeletionVectorStoredBitmap(
    dvDescriptor: DeletionVectorDescriptor,
    tableDataPath: Path
) {
  require(dvDescriptor.isOnDisk, "Only on-disk deletion vectors are supported")

  def load(dvStore: RapidsDeletionVectorStore): HostMemoryBuffer = {
    val buffer = if (isEmpty) {
      RapidsDeletionVectorStoredBitmap.EMPTY_BITMAP
    } else {
      if (isInline) {
//        val serializedStandardBitmapSize =
//          dvDescriptor.inlineData.size - DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE
//        val serializedBitmap = HostMemoryBuffer.allocate(serializedStandardBitmapSize)
//        withResource(new ByteArrayInputStream(dvDescriptor.inlineData,
//          DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE, serializedStandardBitmapSize)) { bais =>
//          serializedBitmap.copyFromStream(0, bais, serializedStandardBitmapSize)
//        }
//        serializedBitmap
        throw new UnsupportedOperationException("Inline deletion vectors are not supported")
      } else {
        assert(isOnDisk)
        withResource(dvStore.open(onDiskPath)) { inputStream =>
          DeltaSerializedBitmapReader.read(inputStream, dvDescriptor.sizeInBytes)
        }
        dvStore.read(onDiskPath,
          dvDescriptor.offset.getOrElse(0)
            + RapidsDeletionVectorStoredBitmap.DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE,
          dvDescriptor.sizeInBytes)
      }
    }

    buffer
  }

  def size: Int = dvDescriptor.sizeInBytes

  def cardinality: Long = dvDescriptor.cardinality

  lazy val getUniqueId: String = dvDescriptor.serializeToBase64()

  private def isEmpty: Boolean = dvDescriptor.isEmpty

  private def isInline: Boolean = dvDescriptor.isInline

  private def isOnDisk: Boolean = dvDescriptor.isOnDisk

  /** The absolute path for on-disk deletion vectors. */
  private lazy val onDiskPath: Path = dvDescriptor.absolutePath(tableDataPath)
}

object RapidsDeletionVectorStoredBitmap {

  /**
   * The "Delta" roaring bitmap serialization formats begin with a 4-byte magic number.
   * When converting to the "standard" roaring bitmap serialization format, this magic number
   * should be stripped. For details, see:
   * https://github.com/delta-io/delta/blob/ccd3092da05a68027bf9be9ec4273a810b4b9ef3/spark/src/main/scala/org/apache/spark/sql/delta/deletionvectors/RoaringBitmapArray.scala#L512-L515
   */
  val DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE = 4

  /**
   * A serialized empty bitmap in host memory buffer. For details of the serialization format, see:
   * https://github.com/RoaringBitmap/RoaringFormatSpec/blob/8c4f7c7087c2a3a4fa560a34c669be673264f3ad/README.md#extension-for-64-bit-implementations
   */
  lazy val EMPTY_BITMAP: HostMemoryBuffer = {
    val buffer = HostMemoryBuffer.allocate(8)
    buffer.setLong(0, 0L)
    buffer
  }
}

trait DeltaSerializedBitmapReader {
  // TODO: handle exception from read
  def read(): HostMemoryBuffer
}

object DeltaSerializedBitmapReader extends Logging {
  def read(input: DataInputStream, size: Int): HostMemoryBuffer = {
    val magicNumberBuf = HostMemoryBuffer.allocate(
      RapidsDeletionVectorStoredBitmap.DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE, false)
    magicNumberBuf.copyFromStream(0, input,
      RapidsDeletionVectorStoredBitmap.DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE)
    val magicNumber = magicNumberBuf.getInt(0)
    logError("magicNumber in hex: " + Integer.toHexString(magicNumber))
    magicNumber match {
      case PortableRoaringBitmapArraySerializationFormat.MAGIC_NUMBER =>
        DeltaPortableFormatReader(input,
          size - RapidsDeletionVectorStoredBitmap.DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE).read()
      case NativeRoaringBitmapArraySerializationFormat.MAGIC_NUMBER =>
        // TODO: reset is weird.
        input.reset()
        DeltaNativeFormatReader(input, size).read()
      case _ =>
        throw new IOException(s"Unexpected RoaringBitmapArray magic number $magicNumber")
    }
  }
}

case class DeltaPortableFormatReader(input: InputStream, bitmapSize: Int)
  extends DeltaSerializedBitmapReader {

  override def read(): HostMemoryBuffer = {
    val buffer = HostMemoryBuffer.allocate(bitmapSize)
    buffer.copyFromStream(0, input, bitmapSize)
    buffer
  }
}

case class DeltaNativeFormatReader(input: DataInputStream, size: Int)
  extends DeltaSerializedBitmapReader {

  override def read(): HostMemoryBuffer = {
    val originalBytes = DeletionVectorStore.readRangeFromStream(input, size)
    val roaringBitmapArray = RoaringBitmapArray.readFrom(originalBytes)
    val reserialized = roaringBitmapArray.serializeAsByteArray(RoaringBitmapArrayFormat.Portable)
    val magicNumberSize = RapidsDeletionVectorStoredBitmap.DELTA_BITMAP_MAGIC_NUMBER_BYTE_SIZE
    val buffer = HostMemoryBuffer.allocate(reserialized.length - magicNumberSize)
    buffer.setBytes(0, reserialized, magicNumberSize, reserialized.length - magicNumberSize)
    buffer
  }
}
